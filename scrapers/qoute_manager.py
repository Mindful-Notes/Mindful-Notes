        ...
        for set_idx in range(max_sets):
            ...
            for page_num in range(start_page, end_page + 1):
                ...
                try:
                    ...
                    if not nodes:
                        print(
                            f" {page_num}페이지에 데이터가 없습니다. 수집종료합니다."
                        )
                    ...
                    for node in nodes:
                        ...
                        if len(parts) >= 2:
                            ...
                            # author = match[0] if match else "작자미상"
                            words = parts[1].strip().split()
                            author = " ".join(words[:2]) if words else "작자미상"
                        else:
                            author = "작자미상"
    ...
    async def sync_questions(self):
        ...
        for node in parser.css(".MarkdownViewer.Markdown ol li"):
            ...
            try:
                ...
                await REFLECTION_QUESTIONS.get_or_create(contents=question_in.contents)
                count += 1
